...
def create_plot_generation_code(plot_type: str = "sine_wave", timestamp: int = None) -> str:
    """
...
class TestContainerFilesystem:
    ...
    def test_given_container_when_creating_multiple_plots_then_all_persist(
        ...
        for plot_type, result in plot_results:
            assert result["success"] is True, f"{plot_type} plot creation failed: {result.get('error')}"
            ...
            if result["data"]["result"]:
                plot_result = eval(result["data"]["result"])
                assert plot_result["file_exists"] is True, f"{plot_type} plot file should exist"
                assert plot_result["file_size"] > 0, f"{plot_type} plot should have content"
...
'''
        ...
        if stdout:
            try:
                ...
                assert dir_results["success"] is True, f"Directory operations failed: {dir_results.get('errors')}"
                assert len(dir_results["created_dirs"]) > 0, "Should have created directories"
                assert len(dir_results["created_files"]) > 0, "Should have created test files"
                
                # Validate specific directories
                created_dirs = dir_results["created_dirs"]
                assert any("/container_test/level1" in d for d in created_dirs)
                assert any("/container_test/plots/matplotlib" in d for d in created_dirs)
...
'''
        ...
        if stdout:
            try:
                ...
                assert error_results["successful_operations"] >= 0, "Should track successful operations"
                ...
                assert summary["system_stable"] is True, "System should remain stable after errors"
                
                # Check specific operations
                tests = error_results["tests"]
                valid_test = next((t for t in tests if t["test"] == "valid_file_creation"), None)
                if valid_test:
                    assert valid_test["success"] is True, "Valid file creation should succeed"
