        ...
        def _validate_api_contract(self, response_data: Dict[str, Any]) -> None:
            ...
            if response_data["success"]:
                ...
                assert isinstance(data["executionTime"], (int, float)), \
                    f"executionTime must be number: {type(data['executionTime'])}"
...
def test_configuration_constants_properly_defined():
    ...
    assert TestConfig.TIMEOUTS["code_execution"] > TestConfig.TIMEOUTS["api_request"], \
        "Code execution timeout should be longer than API timeout"
