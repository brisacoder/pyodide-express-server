...
"NumPy operations completed successfully"
    ...
    assert "NumPy version:" in result_text
    assert "Array: [1 2 3 4 5]" in result_text
    assert "Sum: 15" in result_text
    assert "Mean: 3.0" in result_text
    assert "NumPy operations completed successfully" in result_text
...
"Complex string handling completed"
    ...
    assert "String 1: 'Simple string'" in result_text
    assert 'String 2: \'Single quotes with "double inside"\'' in result_text
    assert "Total test strings: 7" in result_text
    assert "Complex string handling completed" in result_text
...
"Pandas operations completed successfully"
    ...
    response = api_client.post(
        f"{base_url}{TestConfig.EXECUTE_RAW_ENDPOINT}",
        json={"code": code, "timeout": default_timeout}
    )
...
def broken_function(
    ...
    response = api_client.post(
        f"{base_url}{TestConfig.EXECUTE_RAW_ENDPOINT}",
        json={"code": code, "timeout": short_timeout}
    )
...
def test_given_parametric_code_when_executing_then_should_return_expected_output(
    ...
    response = api_client.post(
        f"{base_url}{TestConfig.EXECUTE_RAW_ENDPOINT}",
        json={"code": test_case, "timeout": default_timeout}
    )
...
"Large output generation completed"
    ...
    response = api_client.post(
        f"{base_url}{TestConfig.EXECUTE_RAW_ENDPOINT}",
        json={"code": code, "timeout": long_timeout}
    )
