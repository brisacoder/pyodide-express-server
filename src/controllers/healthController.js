...
async function getDetailedHealthStatus() {
  ...
  return {
    ...
    pyodide: pyodideStatus.status === 'fulfilled' ? pyodideStatus.value : { error: pyodideStatus.reason?.message },
    system: systemInfo.status === 'fulfilled' ? systemInfo.value : { error: systemInfo.reason?.message },
    disk: diskInfo.status === 'fulfilled' ? diskInfo.value : { error: diskInfo.reason?.message },
    directories: directoryInfo.status === 'fulfilled' ? directoryInfo.value : { error: directoryInfo.reason?.message },
...
async function testPyodideBasic() {
  try {
    ...
    const isSuccessful = executionResult.success && 
                        (executionResult.result === 4 || executionResult.result === '4');
